...
int main()
{
    ...
    for(int i = 0; i < 27; i++)
    {
        if(arr_big[i]!=0)
        {
            arr_small[count2++] = arr_big[i];	//统计有在字符中出现的字母的出现次数 
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  //判断是否为叶子节点 
        {
            length = findParent(i,huffman,count);   //得到各个叶结点的路径长度
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  //当遇到还没赋权重的点时跳出循环
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1)  //循环比较权重大小 
        {
            min = huffman[i][3];
            ii[0] = i;
        }
    ...
    for(int j = 0; j < 2 * n - 1; j++)
    {
        ...
        if(huffman[j][3] < min2 && ii[0]!=j && huffman[j][0] == -1)  
        {
            min2 = huffman[j][3];
            ii[1] = j;
        }
...
{
    int length = 0;
    if(huffman[i][0] == -1)     //判断是否根节点，根节点到根节点路径长度即为0 
    {
        return 0;
    }
...
2. 该代码可以对大写字母的字符串进行huffman编码，如果要对小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？   
   答：小写字母ASCII码十六进制为0x61~0x61+25。若要对小写字母字符串进行编码，则在遍历字符串所有字符、计算字母个数时将0x41改为0x61。若想要对同时具有大写和小写的字符串进行编码，由于Z与aASCII码非连续，'Z' == 0x41+25 , 'a' = 0x41+32 ,则设str[i] = char(0x41+i) , i<26 || i>31。循环时设定循环变量小于27也需更改
